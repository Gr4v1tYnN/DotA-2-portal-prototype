...
<head>
	...
	<meta charset="utf-8">
	<title>Edit Match #<?php echo $match[0]['matchinfo_id']; ?>  | DDoser</title>
...
<?php if ($this->session->userdata('status') == 1): ?>
	...
	<div class="container content" style="background-color: #e6e6e6;">
		<div class="card">
			<div class="card-head" style="font-size:30px; margin: 0px 0px 15px 0px">Edit Article</div>
		</div>

		<form action="http://localhost/diplominis/adm/editmatch" method="post">
			<input type="hidden" name="matchId" value="<?php echo $match[0]['matchinfo_id']; ?>">
			<div class="form-group">
				<label for="title">Tournament</label><br>
				<input type="text" name="tournament" value="<?php echo $match[0]['tournament_name']; ?>" required>
			</div>
			<div class="form-group">
				<label for="header">Team one</label><br>
				<input type="text" name="team1" value="<?php echo $match[0]['team1_name']; ?>">
			</div>
			<div class="form-group">
				<label for="abody">Team two</label><br>
				<input type="text" name="team2" value="<?php echo $team2[0]['team2_name']; ?>">
			</div>
			<div class="form-group">
				<label for="matchDate">Date</label><br>
				<input type="date" name="matchDate" value="<?php echo $match[0]['match_date']; ?>" required>
			</div>
			<div class="form-group">
				<label for="matchTime">Time</label><br>
				<input type="time" name="matchTime" value="<?php echo $match[0]['match_time']; ?>" required>
			</div>
			<div class="form-group">
				<label for="matchTime">Streams</label><br>
				<input type="text" name="streams" value="<?php echo $match[0]['streams']; ?>">
			</div>
			<input type="submit" class="floated btn btn-success" value="Submit">
		</form>
		<form action="http://localhost/diplominis/adm/cancelmatchedit" method="post">
			<input type="hidden" name="cancelId" value="<?php echo $match[0]['matchinfo_id']; ?>">
