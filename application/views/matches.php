...
<head>
	...
	<meta charset="utf-8">
	<title>DotA 2 Matches & Results | DDoser</title>
...
<div class="container content" style="background-color: #e6e6e6;">
	<div class="card">
		<div class="card-head" style="font-size:30px; margin: 0px 0px 15px 0px">Matches</div>
	</div>
	<?php if ($this->session->userdata('status') == 1): ?>
		<form action="http://localhost/diplominis/adm/creatematch">
	...
	<div class="tab">
		...
		<button class="tablinks" onclick="Display(event, 'Results')">Results</button>
	</div>
				...
				<?php if ( ($match['match_date'] > date("Y-m-d")) || ($match['match_date'] == date("Y-m-d") && ($match['match_time'] >= date("h:i:s"))) ): ?>
					...
					<table class="table">
						<tr>
							...
							<td style="width: 80px">
								<img style="mix-blend-mode: multiply" src="<?php echo $match['team2_image'];?>" height="40" width="40">
							</td>
							...
							<td style="color: black;">
								<p style="font-size:15px; margin: 8px 0px 0px auto"><?php echo $match['match_status'];?></p>
							</td>
						<tr>
					</table>
					</a>
	...
	<div id="Results" class="tabcontent">
		<?php foreach ($matchese AS $key=>$match): ?>
			<?php if($pageFrom <=  $key && $pageTo > $key){?>
				<?php if (!( ($match['match_date'] > date("Y-m-d")) || ($match['match_date'] == date("Y-m-d") && ($match['match_time'] >= date("h:i:s"))) ) ): ?>
				<a href="http://localhost/diplominis/matchinfo/index?id=<?php echo $match['matchinfo_id'];?>" style=" text-decoration: none;">
				<table class="table">
					<tr>
						...
						<td style="color: black; width: 25px">
								<p style="font-size:15px; margin: 8px 0px 0px auto"><strong><?php echo $match['scoret1'];?></strong></p>
							</td>
						<td style="color: black; width: 25px">
							<p style="font-size:15px; margin: 8px 0px 0px auto"><strong>vs</strong></p>
						</td>
						<td style="color: black; width: 25px">
								<p style="font-size:15px; margin: 8px 0px 0px auto"><strong><?php echo $match['scoret2'];?></strong></p>
							</td>
						<td style="color: black; width: 250px">
							<p style="font-size:15px; margin: 8px 0px 0px auto"><strong><?php echo $match['team2_name'];?></strong></p>
						</td>
...
<div class="container">	
	<ul class="nav nav-tabs">
		<?php if($page != 1 && $page != 0){?>
			<li class="nav-item">
			  <a class="nav-link" href="http://localhost/diplominis/matches?page=<?php echo $page-1; ?>"
				role="tab" data-toggle="tab"><</a>
			</li>
		<?php }?>
		<li class="nav-item">
		  <a class="nav-link" href="http://localhost/diplominis/matches?page=<?php echo $page+1; ?>" role="tab"
			data-toggle="tab">></a>
		</li>
	</ul>
</div>
...
<script>
function Display(evt, showMatches) {
  ...
  document.getElementById(showMatches).style.display = "block";
