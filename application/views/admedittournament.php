...
<head>
	...
	<meta charset="utf-8">
	<title>Edit Tournament #<?php echo $tournament[0]['tournamentinfo_id']; ?>  | DDoser</title>
...
<?php if ($this->session->userdata('status') == 1): ?>
	...
	<div class="container content" style="background-color: #e6e6e6;">
		<div class="card">
			<div class="card-head" style="font-size:30px; margin: 0px 0px 15px 0px">Edit Tournament</div>
		</div>

		<form action="http://localhost/diplominis/adm/edittournament" method="post">
		<input type="hidden" name="tournamentId" value="<?php echo $tournament[0]['tournamentinfo_id']; ?>">
		<div class="form-group">
			<label for="name">Tournament name</label><br>
			<input type="text" name="name" value="<?php echo $tournament[0]['tournament_name']; ?>" required>
		</div>
		<div class="form-group">
			<label for="dateStart">Starting date</label><br>
			<input type="date" name="dateStart" value="<?php echo $tournament[0]['date_start']; ?>">
		</div>
		<div class="form-group">
			<label for="dateFinish">Ending date</label><br>
			<input type="date" name="dateFinish" value="<?php echo $tournament[0]['date_finish']; ?>">
		</div>
		<div class="form-group">
			<label for="prizepool">Prizepool</label><br>
			<input type="number" name="prizepool" value="<?php echo $tournament[0]['prizepool']; ?>">
		</div>
		<div class="form-group">
			<label for="location">Location</label><br>
			<input type="text" name="location" value="<?php echo $tournament[0]['location']; ?>">
		</div>
		<div class="form-group">
			<label for="participantsNumber">Number of Participants</label><br>
			<input type="number" name="participantsNumber" value="<?php echo $tournament[0]['participants_num']; ?>">
		</div>
		<div class="form-group">
			<label for="participants">Participants</label><br>
			<input type="text" name="participants" value="<?php echo $tournament[0]['participants']; ?>">
		</div>
	  <input type="submit" class="floated btn btn-success" value="Submit">
	</form>
	<form action="http://localhost/diplominis/adm/canceltournamentedit" method="post">
		<input type="hidden" name="cancelId" value="<?php echo $tournament[0]['tournamentinfo_id']; ?>">
